...
import { Camera, Search, BookOpen, ListTodo, CheckCircle2, Circle, ChevronLeft, X, ScanLine, Type, AlertCircle } from 'lucide-react';

// Base de datos completa - 48 Equipos clasificados por grupos
const TEAMS = [
...
export default function App() {
  ...
  const totalStickers = 960; // 48 equipos * 20
  ...
  const renderContent = () => {
    if (selectedCountry && activeTab === 'album') {
      ...
      // Salvavidas: si el equipo no se encuentra (por transición), vuelve al inicio
      if (!team) {
        setSelectedCountry(null);
      ...
      return <CountryView team={team} onBack={handleBack} collection={collection} toggleSticker={toggleSticker} />;
    }
    ...
    switch (activeTab) {
      case 'album':
        return <HomeView searchQuery={searchQuery} setSearchQuery={setSearchQuery} onSelectCountry={setSelectedCountry} collection={collection} progressPercent={progressPercent} collectedCount={collectedCount} totalStickers={totalStickers} />;
      case 'missing':
        return <MissingView collection={collection} toggleSticker={toggleSticker} />;
      case 'scanner':
        return <ScannerView onScanComplete={(id) => { toggleSticker(id); setActiveTab('missing'); }} collection={collection} />;
      default:
        return null;
    }
  };
...
function HomeView({ searchQuery, setSearchQuery, onSelectCountry, collection, progressPercent, collectedCount, totalStickers }) {
  ...
  // Cálculo de círculo seguro (evita decimales infinitos que causan crasheos)
  const circleDasharray = `${((progressPercent / 100) * 138).toFixed(1)} 138`;

  return (
    <div className="animate-in fade-in duration-300 min-h-full bg-gray-50 pb-8">
      ...
      <div className="px-5 space-y-6">
        ...
        <div className="relative">
          ...
          <input
            type="text"
            className="block w-full pl-11 pr-4 py-3.5 border-none rounded-2xl bg-white shadow-sm placeholder-gray-400 focus:ring-2 focus:ring-[#2D338B] text-sm font-medium"
            placeholder="Buscar país o código (ej. MEX)..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
          />
          {searchQuery && <button onClick={() => setSearchQuery('')} className="absolute inset-y-0 right-0 pr-4 flex items-center text-gray-400"><X size={18} /></button>}
        </div>
        ...
        <div className="space-y-8">
          {GROUPS.map(groupLetter => {
            ...
            return (
              <div key={groupLetter} className="animate-in fade-in">
...
function CountryView({ team, onBack, collection, toggleSticker }) {
  if (!team) return null; // Salvavidas
  const stickers = Array.from({ length: team.count }, (_, i) => i + 1);

  return (
    <div className="animate-in slide-in-from-right-4 duration-200 min-h-full bg-gray-50 pb-6">
...
function ScannerView({ onScanComplete, collection }) {
  ...
  const handleCapture = () => {
    setIsScanning(true);
    // Limpiar timeout anterior por seguridad
    if (scanTimeoutRef.current) clearTimeout(scanTimeoutRef.current);
  ...
  return (
    <div className="h-full bg-black relative flex flex-col animate-in fade-in duration-300">
      <div className="relative flex-1 bg-[#1A1A1A] overflow-hidden flex items-center justify-center">
        {hasCamera ? (
          ...
          <div className="text-white text-center p-8 flex flex-col items-center justify-center w-full h-full bg-gray-900">
            ...
            <p className="text-sm text-gray-400 mb-6 text-center">El navegador de prueba ha bloqueado el acceso a la cámara por seguridad.</p>
            <div className="bg-[#2D338B]/30 border border-[#2D338B] p-4 rounded-2xl w-full max-w-xs">
              <p className="text-xs text-[#00A3E0] font-bold text-center leading-relaxed">
                ¡No te preocupes! El botón rojo funcionará en <span className="text-white">MODO SIMULADO</span> para probar la app.
              </p>
            </div>
          </div>
        ) : (
        ...
        <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
          ...
          {isScanning && <div className="absolute top-0 left-0 right-0 h-1 bg-[#00A3E0] shadow-[0_0_20px_rgba(0,163,224,1)] animate-[scan_2s_ease-in-out_infinite]"></div>}
        </div>

        {extractedData && !isScanning && (
          <div className="absolute inset-0 flex items-end justify-center z-20 pb-4 bg-black/60 backdrop-blur-md">
            <div className="bg-white p-6 rounded-[2rem] w-[90%] shadow-2xl animate-in slide-in-from-bottom-8">
      ...
      <style>{`@keyframes scan { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(600px); } }`}</style>
    </div>
  );
}